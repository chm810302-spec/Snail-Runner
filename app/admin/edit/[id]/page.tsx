...
export default function EditPostPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function fetchPost() {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setTitle(data.title || "");
          setCategory(data.category || "Gear Review");
  ...
  }, [resolvedParams.id, isAdmin, router]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(doc(db, "posts", resolvedParams.id), {
        title,
        category,
        excerpt,
        content,
      ...
      const errorString = error?.message?.toLowerCase() || String(error).toLowerCase();
      if (errorString.includes("missing or insufficient permissions")) {
        setErrorMsg("You don't have permission to update this post.");
